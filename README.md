...
Необходимо создать React-компоненты `ShopItemFunc` (функциональный компонент) и `ShopItemClass` (class-based компонент), с помощью которых мы могли бы реализовывать представление информации о товарах из нашего каталога на сайте в таком виде (компонент обведён пунктирной линией):
